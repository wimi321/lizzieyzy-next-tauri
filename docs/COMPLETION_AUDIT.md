...
| Readboard, OCR, and capture | Scoped partial pass | Readboard sidecar probe/protocol evidence, controlled image import path/base64 evidence, controlled OCR corpus evidence, local-image backend decode evidence, operator-selected preview/confirm/import evidence, and controlled local target-window screenshot fixture evidence, and runtime-backed controlled arbitrary screenshot board-region OCR evidence are recorded. | Not arbitrary live screenshot OCR, not target-client discovery, not real external client/window parity, not full readboard parity, and not full legacy OCR parity. | Real arbitrary capture/OCR and target-client discovery are not proven. | Add runtime gates for arbitrary live screenshots, external windows/clients, target-client metadata, cross-platform capture, and legacy OCR fixture comparison. |
...
3. Convert scoped OCR/capture MVP, controlled target-window fixture evidence, and runtime-backed controlled arbitrary screenshot board-region OCR evidence into real target-client and arbitrary live screenshot validation.
