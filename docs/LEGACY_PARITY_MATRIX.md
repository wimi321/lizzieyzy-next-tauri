...
| SGF tree, variations, and comments panel | `partial` | Acceptance requires API/DTO tests for stable tree node ids, parent/child variation links, move indexes, selected-node lookup, comment text exposure/save, branch node position replay, and UI smoke showing tree navigation, variation selection, comments panel updates, comment save, and branch position display from golden SGF fixtures. | UI automation must prove comment save, branch navigation, and SGF reopen round-trip before broader edit parity is claimed. | Comment save and branch node position foundation are implemented for this batch, but adding, deleting, or reordering variations, full annotation editing, and broader SGF editing parity remain incomplete. |
