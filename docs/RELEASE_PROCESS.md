...
The workflow runs a macOS, Linux, and Windows matrix that checks out the repository, installs Node/Rust tooling, installs Linux Tauri system dependencies where needed, validates `tauri.conf.json`, runs `python3 scripts/validate_scaffold.py --verbose`, builds the frontend, and runs `npm run tauri:build -- --no-bundle --ci --no-sign` unless disabled for a manual run.
...
Release notes must not claim full legacy parity or Fox/Yike/readboard support until those workflows have explicit acceptance evidence.
