...
| SGF tree, variations, and comments panel | `partial` | Acceptance requires API/DTO tests for stable tree node ids, parent/child variation links, move indexes, selected-node lookup, comment text exposure/save, branch node position replay, append move/pass as child or sibling variation, delete selected non-root node/subtree behavior, and UI smoke showing tree navigation, variation selection, comments panel updates, comment save, branch position display, append move/pass, delete selected non-root subtree, and SGF reopen round-trip behavior from golden SGF fixtures. | UI automation must prove comment save, branch navigation, append move/pass, delete selected non-root subtree, and SGF reopen round-trip before broader edit parity is claimed. | Supports comment save, branch position replay, append move/pass as a child or sibling variation, and delete selected non-root node/subtree as an SGF tree editing foundation. Reorder variations, edit existing moves/properties, full annotation editing, legacy import/capture helpers, and broader SGF editing parity remain incomplete. |
...
| Editing, annotation, and legacy import/capture helpers | `missing` | Acceptance requires tests and smoke evidence for editing moves/properties/comments, annotation persistence, and legacy capture/import helpers beyond the current SGF flows. | Legacy fixture comparison for migrated helper behavior. | Append move/pass and delete selected non-root node/subtree foundations are tracked in the SGF tree row only. Edit existing moves/properties, reorder variations, full annotation editing, and legacy import/capture helpers remain incomplete, so this broader row stays missing. |
