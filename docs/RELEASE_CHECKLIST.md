...
- The repository smoke gate currently has zero pending items after scoped manual-assisted native desktop SGF open/edit/save/reopen workflow evidence and scoped KataGo review workflow UX resilience evidence are recorded. Signed/notarized release proof, updater readiness, release publication, store distribution, Windows/Linux installed-app proof, WebView DOM proof, live KataGo review workflow proof, OCR/capture helpers, provider/readboard parity, full legacy analysis parity, and full legacy parity remain outside the existing recorded proof.
- The scoped legacy import/capture helper surface does not change release readiness for real OCR or external capture. Real image OCR, target client/window capture, and full legacy helper migration remain external gates.
