...
Windows/Linux installed-app smoke is tracked as future per-platform evidence at `docs/qa/windows-unsigned-installed-app-smoke.json` and `docs/qa/linux-unsigned-installed-app-smoke.json`. No evidence JSON is committed yet and this is not a central smoke PASS. A future scoped PASS must come from CI runner output and include artifact hash/size, launch command, process/window observation, dev-server absence, exit/terminate success, display mode, and explicit false boundaries for production signing, updater readiness, official release publication, Windows/Linux installed-app parity, full release parity, and full legacy parity.
...
- Future per-platform evidence must be committed as `docs/qa/windows-unsigned-installed-app-smoke.json` and `docs/qa/linux-unsigned-installed-app-smoke.json` after CI runner output records each platform's artifact path/hash/size, launch command, process/window observation, dev-server absence, exit/terminate success, and display mode. Until those files exist with PASS evidence, this is tooling/workflow preparation only; it is not signing, updater readiness, official release publication, Windows/Linux parity, full release parity, or full legacy parity.
...
- The repository smoke gate currently has zero pending items after scoped controlled readboard image OCR corpus evidence at `docs/qa/readboard-image-ocr-corpus-smoke-macos.json` is recorded. Signed/notarized release proof, updater readiness, release publication, store distribution, Windows/Linux installed-app proof, full Tauri WebView layout/shortcut/release parity, full legacy UI/layout parity, full legacy analysis parity, arbitrary screenshot OCR, external window/client capture, real client capture, provider/readboard parity, full legacy OCR parity, full readboard parity, and full legacy parity remain outside the existing recorded proof.
