...
| Readboard, OCR, and capture | Scoped partial pass | Readboard sidecar probe/protocol evidence, controlled image import path/base64 evidence, controlled OCR corpus evidence, local-image backend decode evidence, operator-selected preview/confirm/import evidence, and controlled local target-window screenshot fixture evidence are recorded. | Not arbitrary live screenshot OCR, not target-client discovery, not real external client/window parity, not full readboard parity, and not full legacy OCR parity. | Real arbitrary capture/OCR and target-client discovery are not proven. | Add runtime gates for arbitrary live screenshots, external windows/clients, target-client metadata, cross-platform capture, and legacy OCR fixture comparison. |
...
- Arbitrary live screenshot OCR, target-client discovery, and real external client/window capture have not been proven.
...
3. Convert scoped OCR/capture MVP and controlled target-window fixture evidence into real target-client and arbitrary live screenshot validation.
