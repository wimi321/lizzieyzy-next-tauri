...
- `python3 scripts/smoke_user_flows.py --verbose` currently reports `40 passed, 0 failed, 0 pending`; repository-local native SGF save/read-back refresh, existing-move edit surface evidence, scoped annotation UI/API evidence, scoped legacy import/capture helper surface evidence, scoped Desktop SGF Editing UX source-surface/runtime-chain evidence, scoped browser-rendered DOM/click/screenshot/menu-action evidence, scoped Tauri runtime/window screenshot evidence, scoped Tauri WebView DOM/click evidence, scoped installed macOS `.app` launch/window evidence, scoped manual-assisted native desktop SGF workflow evidence, scoped legacy config migration transactional/rollback safety surface evidence, scoped bundled/runtime asset layout surface evidence, scoped macOS local Tauri runtime UI evidence, scoped macOS live KataGo evidence, scoped KataGo review workflow UX resilience evidence, scoped macOS readboard runtime evidence, scoped readboard controlled image import MVP evidence, scoped macOS provider controlled-network evidence, and scoped multiplatform packaging smoke evidence are complete for their current gates; scoped macOS native menu/shortcut evidence is complete for its current gate. The helper surface records visible SGF/payload and protocol snapshot helper paths plus recoverable unsupported OCR/image and external capture states without importing SGF or replacing the board. The macOS local Tauri runtime UI evidence is recorded with `annotation_edit` and reopened `afterReopen.annotationsVerified`; the Desktop SGF Editing UX evidence records source-visible LegacyShell/tree/annotation/dirty-saved UX fields with `runtimeDomObserved=false`, `screenshotObserved=false`, and `nativeDialogClickCovered=false`; the browser click evidence records scoped browser DOM/click/screenshot observation and LegacyShell menu action active-target proof while excluding native file dialog, full shortcut/layout parity, and Tauri WebView DOM proof; the native menu/shortcut evidence records scoped macOS native menu surface, event bridge, keyboard shortcut surface, aligned action ids, and input-editing safety while excluding full shortcut parity, full legacy menu parity, WebView DOM proof, full OS-native menu parity, release/signing/notarization, provider/readboard/OCR, and Windows/Linux claims; the KataGo review workflow UX evidence records source-static plus stubbed UI-flow resilience with `liveKataGoObserved=false`, `fullLegacyAnalysisParity=false`, and no release/provider/readboard/OCR/Windows/Linux claims; the readboard image import evidence records controlled path/base64 image import, invalid/non-board rejection, snapshot/board-size/stone-count/to-play verification, protocol regression, `fullOcrParity=false`, and `externalCaptureCovered=false`; the Tauri window evidence records scoped Tauri runtime/window screenshots while excluding browser fallback, WebView DOM clicks, and native dialog clicks; the Tauri WebView DOM/click evidence records runtime DOM/click/visible-target proof while excluding browser fallback, full layout parity, full shortcut parity, full legacy parity, release parity, and OCR capture parity; the installed macOS app evidence records scoped packaged `.app` launch/window proof while excluding signing, notarization, and release publication; the native desktop SGF workflow evidence records manual-assisted native open/edit/save/reopen proof while excluding full automation, signing, notarization, release publication, Windows/Linux installed-app proof, arbitrary screenshot OCR, external window/client capture, full legacy OCR parity, provider/readboard proof, and full legacy parity.
...
- The repository smoke gate currently has zero pending items after scoped manual-assisted native desktop SGF open/edit/save/reopen workflow evidence, scoped Tauri WebView DOM/click evidence, scoped KataGo review workflow UX resilience evidence, and scoped readboard controlled image import MVP evidence are recorded. Signed/notarized release proof, updater readiness, release publication, store distribution, Windows/Linux installed-app proof, full Tauri WebView layout/shortcut/release parity, live KataGo review workflow proof, arbitrary screenshot OCR, external window/client capture, provider/readboard parity, full legacy analysis parity, full legacy OCR parity, and full legacy parity remain outside the existing recorded proof.
...
Pass: both live evidence JSON files are sanitized and have status `pass`; the CLI evidence records one-position and batch `katago analysis` responses, and the Tauri evidence records runtime startup, assets, analyze-once, analyze-game, and start/cancel. The scoped UX resilience evidence is separately sanitized with `liveKataGoObserved=false` and proves only source-static plus stubbed UI-flow behavior for progress, cancel/restart, cache restore, engine failure message, and stale-analysis guard. `smoke_user_flows.py` reports `katago_live_smoke` and `katago_review_workflow_ux_smoke` as PASS only after their respective evidence files pass. This is macOS local live KataGo CLI plus Tauri runtime evidence and scoped UX resilience evidence; it is not live cache-hit proof, full legacy analysis parity, bundled-engine proof, or multiplatform packaging proof.
...
For scoped legacy Java/Swing config migration transactional/rollback safety-surface evidence, verify the Preferences panel exposes the migration controls and run:

```bash
python3 scripts/smoke_user_flows.py --verbose
```

Pass: `legacy_config_migration_surface` reports PASS, proving repository-local frontend/API wiring for legacy config path input, Preview, Apply, status, warnings, migrated fields, structured apply status/error, written target labels, transactional/no-write flags, rollback state, rollback paths/errors, and backend calls to `preview_legacy_config_migration`/`apply_legacy_config_migration`. This is scoped transactional/rollback safety-surface evidence; it is not broad legacy config corpus migration proof, real rollback-failure exercise, or a claim that every Java/Swing setting migrates.
