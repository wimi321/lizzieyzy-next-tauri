...
| SGF tree, variations, and comments panel | `partial` | Acceptance requires API/DTO tests for stable tree node ids, parent/child variation links, move indexes, selected-node lookup, and comment text exposure, plus UI smoke showing tree navigation, variation selection, and comments panel updates from golden SGF fixtures. | UI automation must prove comment save and SGF reopen round-trip before edit parity is claimed. | Tree DTO/API/UI foundation may count only as partial; comment editing, save, and round-trip persistence are not implemented unless backed by tests and desktop smoke evidence. |
