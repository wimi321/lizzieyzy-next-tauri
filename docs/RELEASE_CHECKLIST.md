...
- `docs/qa/katago-analysis-stale-guard-smoke-macos.json` is the scoped KataGo analysis stale-guard evidence target. The repository gate requires schema `lizzieyzy.katago-analysis-stale-guard-smoke.v1`, status `pass`, platform `macos`, source facts for `activeAnalysisEditVersionRef`, `beginAnalysisEditGuard`, `isAnalysisEditVersionCurrent`, and `isCurrentAnalysisJob(jobId, editVersion)` comparing both job id and edit version, plus active job cancel/cleanup and stale result ignore. This is CI-verifiable stale-guard evidence only; it is not full legacy analysis parity, full KataGo parity, release parity, signed release parity, bundled large-model parity, provider/readboard parity, or Windows/Linux proof.
...
- The repository smoke gate currently reports `61 passed, 0 failed, 0 pending`, including scoped controlled readboard image OCR corpus evidence, scoped target-window discovery evidence, scoped provider session/error/schema evidence, scoped Windows/Linux unsigned installed-app evidence at `docs/qa/windows-unsigned-installed-app-smoke.json` and `docs/qa/linux-unsigned-installed-app-smoke.json`, and scoped release readiness preflight evidence. Signed/notarized release proof, updater readiness, official release publication, store distribution, Windows/Linux signed/release installed-app proof, full production release validation, bundled large-model parity, full Tauri WebView layout/shortcut/release parity, full legacy UI/layout parity, full legacy analysis parity, arbitrary live screenshot OCR, external window/client capture, real client capture, provider/readboard parity, full legacy OCR parity, full readboard parity, and full legacy parity remain outside the existing recorded proof.
- The scoped legacy import/capture helper surface and scoped controlled image import MVP do not change release readiness for arbitrary live screenshot OCR or external capture. External window/client capture, arbitrary live screenshot OCR, and full legacy helper/OCR parity remain external gates.
- The selected-window capture validator is preparation only. Until runtime evidence is recorded, it does not add another central PASS beyond the current `61 passed, 0 failed, 0 pending` smoke count or change release readiness.
...
Pass: controlled board image import is scoped to the MVP evidence; arbitrary live screenshot OCR, external window/client capture, and full legacy OCR parity remain pending.
