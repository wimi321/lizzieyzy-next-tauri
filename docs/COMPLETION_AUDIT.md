...
| Fox/Yike providers | Scoped controlled-network pass, external validation needed | Controlled-network provider smoke records Yike/Fox fetch paths, typed failure modes, request observation, and offline-parser exclusion. Provider session/error/schema smoke records representative session-expired, auth-required, rate-limit, blocked/anti-bot challenge, schema-changed, empty-result, service-unavailable, and malformed-payload fixtures with typed frontend display evidence. | Not real Fox/Yike service parity, not account/session proof, not anti-bot stability, and not real service schema drift coverage. | Real service credentials/session states and live service behavior are still external. | Run real-service provider smoke with account/session metadata, rate-limit/session-expiry cases, anti-bot/challenge behavior, empty-result handling, and current service schema checks. |
