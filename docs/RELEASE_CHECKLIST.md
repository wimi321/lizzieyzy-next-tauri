...
- The repository smoke gate currently has zero pending items after the scoped browser-rendered DOM/click/screenshot evidence is recorded. Tauri WebView DOM proof, official signing/notarization, release publication, updater readiness, store distribution, native dialog/manual release smoke, platform-specific installed-app release smoke, OCR/capture helpers, and full legacy parity remain outside that proof.
