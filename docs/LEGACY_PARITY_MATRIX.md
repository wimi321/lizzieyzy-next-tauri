...
| SGF tree, variations, and comments panel | `partial` | Acceptance requires API/DTO tests for stable tree node ids, parent/child variation links, move indexes, selected-node lookup, comment text exposure/save, node property exposure/save, branch node position replay, append move/pass as child or sibling variation, delete selected non-root node/subtree behavior, sibling variation reorder behavior, and UI smoke showing tree navigation, variation selection, comments panel updates, comment/property save, branch position display, append move/pass, delete selected non-root subtree, variation reorder, and SGF reopen round-trip behavior from golden SGF fixtures. | UI automation must prove comment/property save, branch navigation, append move/pass, delete selected non-root subtree, variation reorder, and SGF reopen round-trip before broader edit parity is claimed. | Supports comment save, node property update, branch position replay, append move/pass as a child or sibling variation, delete selected non-root node/subtree, and sibling variation reorder as SGF tree/editing foundations. Real desktop reopen smoke, full annotation editing, edit-existing-move workflows, legacy import/capture helpers, and broader SGF editing parity remain incomplete. |
...
| Editing, annotation, and legacy import/capture helpers | `missing` | Acceptance requires tests and smoke evidence for editing existing moves, persisted properties/comments, annotation persistence, variation reorder, and legacy capture/import helpers beyond the current SGF flows. | Legacy fixture comparison for migrated helper behavior plus native desktop save/reopen smoke. | Comment/property update, append move/pass, delete selected non-root node/subtree, and reorder foundations are tracked in the SGF tree row only. Full annotation editing, edit-existing-move workflows, desktop save/reopen proof, and legacy import/capture helpers remain incomplete, so this broader row stays missing. |
