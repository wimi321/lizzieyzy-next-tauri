...
- The repository smoke gate currently has zero pending items after scoped Tauri runtime/window screenshot evidence is recorded. WebView DOM click proof, official signing/notarization, release publication, updater readiness, store distribution, native dialog/manual release smoke, platform-specific installed-app release smoke, OCR/capture helpers, provider/readboard parity, and full legacy parity remain outside that proof.
