...
- The repository smoke gate currently has zero pending items after scoped manual-assisted native desktop SGF open/edit/save/reopen workflow evidence is recorded. Signed/notarized release proof, updater readiness, release publication, store distribution, Windows/Linux installed-app proof, WebView DOM proof, OCR/capture helpers, provider/readboard parity, and full legacy parity remain outside the existing recorded proof.
