...
- The repository smoke gate currently has zero pending items after scoped live KataGo desktop workflow evidence at `docs/qa/katago-live-desktop-workflow-smoke-macos.json` is recorded. Signed/notarized release proof, updater readiness, release publication, store distribution, Windows/Linux installed-app proof, full Tauri WebView layout/shortcut/release parity, full legacy analysis parity, arbitrary screenshot OCR, external window/client capture, provider/readboard parity, full legacy OCR parity, and full legacy parity remain outside the existing recorded proof.
